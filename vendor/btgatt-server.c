...
#define UUID_MS_FF80_SERVICE        0xFF80
#define UUID_MS_FF81_CHARA          0xFF81
#define UUID_MS_FF82_CHARA          0xFF82

#define UUID_MS_FF90_SERVICE        0xFF90
#define UUID_MS_FF91_CHARA          0xFF91
#define UUID_MS_FF92_CHARA          0xFF92
...
struct server {
    ...
    uint16_t ff80_service_handle;           ///FF80
    uint16_t ff81_characteristic_handle;    ///FF81
    uint16_t ff82_characteristic_handle;    ///FF82
    unsigned int ff81_cfg_indicate_timeout_id; //need modify

    uint16_t ff90_service_handle;           ///FF90
    uint16_t ff91_characteristic_handle;    ///FF91
    uint16_t ff92_characteristic_handle;    ///FF92
    unsigned int ff91_cfg_indicate_timeout_id; //need modify
};
...
// extern void ble_uart_receive(uint16_t uuid, uint8_t *buf, int len);
///FF81 write callback
static void ff80_cfg_write_cb(struct gatt_db_attribute *attrib,
                    unsigned int id, uint16_t offset,
                    const uint8_t *value, size_t len,
                    uint8_t opcode, struct bt_att *att,
                    void *user_data)
{
    ...
    printf(" %s: id %d, offset %d, len %ld, opcode %d, data ", __func__, id, offset, len, opcode);
    ...
    // ble_uart_receive(0xFF80,value,len);
    // ble_rx_write_cb((uint8_t*)value, len);
done:
    gatt_db_attribute_write_result(attrib, id, ecode);
}

static void populate_ff80_service(struct server *server)
{
    bt_uuid_t uuid;
    struct gatt_db_attribute *service, *db_att;

    bt_uuid16_create(&uuid, UUID_MS_FF80_SERVICE);
    service = gatt_db_add_service(server->db, &uuid, true, 8);
    server->ff80_service_handle = gatt_db_attribute_get_handle(service);

    bt_uuid16_create(&uuid, UUID_MS_FF81_CHARA);        //FF81 write
    db_att = gatt_db_service_add_characteristic(service, &uuid,
                        BT_ATT_PERM_WRITE,
                        BT_GATT_CHRC_PROP_WRITE,
                        NULL, ff80_cfg_write_cb, 
                        server);
    server->ff81_characteristic_handle = gatt_db_attribute_get_handle(db_att);

    bt_uuid16_create(&uuid, GATT_CHARAC_USER_DESC_UUID); //2901
    gatt_db_service_add_descriptor(service, &uuid,
                    BT_ATT_PERM_READ | BT_ATT_PERM_WRITE,
                    NULL, NULL, NULL);

    bt_uuid16_create(&uuid, UUID_MS_FF82_CHARA);        //FF82 indicate
    db_att = gatt_db_service_add_characteristic(service, &uuid,
                        BT_ATT_PERM_NONE,
                        BT_GATT_CHRC_PROP_INDICATE,     //indicate
                        NULL, NULL, NULL);
    server->ff82_characteristic_handle = gatt_db_attribute_get_handle(db_att);

    bt_uuid16_create(&uuid, GATT_CLIENT_CHARAC_CFG_UUID); ///2902
    gatt_db_service_add_descriptor(service, &uuid,
                    BT_ATT_PERM_READ | BT_ATT_PERM_WRITE,
                    NULL, NULL, NULL);
    ...
    //if (server->hr_visible)
    gatt_db_service_set_active(service, true);
    printf("gatt: create FF80 server ........\r\n");
}

static void populate_ff90_service(struct server *server)
{
    bt_uuid_t uuid;
    struct gatt_db_attribute *service, *db_att;

    bt_uuid16_create(&uuid, UUID_MS_FF90_SERVICE);
    service = gatt_db_add_service(server->db, &uuid, true, 8);
    server->ff90_service_handle = gatt_db_attribute_get_handle(service);

    bt_uuid16_create(&uuid, UUID_MS_FF91_CHARA);        //FF81 write
    db_att = gatt_db_service_add_characteristic(service, &uuid,
                        BT_ATT_PERM_WRITE,
                        BT_GATT_CHRC_PROP_WRITE,
                        NULL, ff80_cfg_write_cb, 
                        server);
    server->ff91_characteristic_handle = gatt_db_attribute_get_handle(db_att);
    ...
    bt_uuid16_create(&uuid, UUID_MS_FF92_CHARA);        //FF82 indicate
    db_att = gatt_db_service_add_characteristic(service, &uuid,
                        BT_ATT_PERM_NONE,
                        BT_GATT_CHRC_PROP_INDICATE,     //indicate
                        NULL, NULL, NULL);
    server->ff92_characteristic_handle = gatt_db_attribute_get_handle(db_att);
    ...
    //if (server->hr_visible)
    gatt_db_service_set_active(service, true);
    printf("gatt: create FF90 server ........\r\n");
}
...
static void populate_db(struct server *server)
{
	...
	populate_ff80_service(server);
	populate_ff90_service(server);
}
...
static struct server *server_create(int fd, uint16_t mtu, bool hr_visible)
{
	...
	if (verbose) {
		bt_att_set_debug(server->att, BT_ATT_DEBUG_HEXDUMP,
							...
							uint8_t src_type)
{
	...
	printf("Connect from %s l2_cid %d l2_bdaddr_type %d sk %d nsk %d\n", ba, addr.l2_cid, addr.l2_bdaddr_type, sk, nsk);
	close(sk);
